...
runcode(function()
	...
	textlabel.Text = "Mods can still ban you with Mont Private "
...
runcode(function()
    local MontAntivoid = {["Enabled"] = false}
    local antivoidpart
    MontAntivoid = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "BlueAntivoid",
        ["HoverText"] = "BlueAntivoid",
        ["Function"] = function(callback)
            if callback then
                task.spawn(function()
                    ...
                    antivoidpart.Color = Color3.fromRGB(0,0,255)
    ...
    RootRemover = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                ...
                createwarning("Mont Private", "Reset to disable", 3)
            end
        end
    })
end)
    ...
    NameHider = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                ...
                createwarning("Mont Private", "Join A New Match To Reset Your Name And Other Names.", 3)
            end
        end
    })
end)
...
runcode(function()
    ...
    BlueChams = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "BlueChams",
        ["HoverText"] = "0x83",
        ["Function"] = function(callback)
                if callback then
                ...
                createwarning("Mont Private", "Disabling Next Game", 3)
            end
        end
    })
end)
...
fastbravity = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
	["Name"] = "FastGravityFly",
	["HoverText"] = "Fly",
	["Function"] = function(callback)
		if callback then
			...
			workspace.Gravity = 192.2
			game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = 20 -- last min changes LOL
		end
	end
})
        ...
        if callback then
            for i, plr in pairs(players:GetChildren()) do
                if plr:IsInGroup(4199740) and plr:GetRankInGroup(4199740) >= 1 then
                    createwarning("Mont Private", "Star found! Name: " .. plr.Name .. "(" .. plr.DisplayName .. ")", 20)
                    end
                end
            end
        end
...
p13:Connect(function(p14)
	if Notify["Enabled"] then
		...
		if team == lplr.Team.Name then
			createwarning("MontPrivate", "bed broke, L", 3.2)
			end
		end
	end)
end)
...
p13:Connect(function(p14)
	if Notify["Enabled"] then
		if p14.player.Name == lplr.Name then
			createwarning("MontPrivate", "you broke a bed! nice", 2)
			end
		end
	end)
end)
...
p6:Connect(function(p7)
	if Notify["Enabled"] then
		if p7.fromEntity and p7.fromEntity == lplr.Character then
			...
			createwarning("MontPrivate", "you killed "..(plr.Name), 3)
			end
		end
	end)
end)
...
FlashingHead = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
    ["Name"] = "Flashing Head",
    ["HoverText"] = "From Stav",
    ["Function"] = function(callback)
        if callback then
            ...
            wait(.0008)
        until (not FlashingHead.Enabled)
    end)
        else
            game.Players.LocalPlayer.Character.Head.Transparency = 0
            createwarning("MontPrivate", "Flashing head show finished", 2)
        end
    end
})

local Messages = {"Hurts!","Bom!","OOOO!","Bom!","Sucks !","Dam!","Bom!","Bam!"}
local theyessirYE = {["Enabled"] = false}
    theyessirYE = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "CustomIndicator",
        ["HoverText"] = "DamageIndicator",
            ["Function"] = function(callback)
                if callback then
                       ...
                       Connection = cam.Viewmodel.ChildAdded:Connect(function(v)
                                if v:FindFirstChild("Handle") then
                                    pcall(function()
                                        ...
                                        v:FindFirstChild("Handle").Color = Color3.fromRGB(195, 17, 255)
                                    end)
                                    local vname = string.lower(v.Name)
                                    if vname:find("sword") or vname:find("blade") then
                                        v:FindFirstChild("Handle").MeshId = "rbxassetid://13277738229"
                                    end
                                end
                            end)
                        else
                       ...
                       Connection = cam.Viewmodel.ChildAdded:Connect(function(v)
                                if v:FindFirstChild("Handle") then
                                    pcall(function()
                                        ...
                                        v:FindFirstChild("Handle").Material = Enum.Material.Neon
                                        v:FindFirstChild("Handle").TextureID = ""
                                        v:FindFirstChild("Handle").Color = Color3.fromRGB(255, 0, 4)
                                    end)
                                    local vname = string.lower(v.Name)
                                    if vname:find("sword") or vname:find("blade") then
...
smootherheatseeker = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "Mont Heatseeker",
    ["Function"] = function(sin)
    if math.sin then
        task.spawn(function()
repeat           
  createwarning("Mont", "boost", 8.73)
