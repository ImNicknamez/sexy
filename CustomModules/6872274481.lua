				...
				if plr then
					...
					if plrtag then
						...
						elseif plrtype == "FAPE PRIVATE" then
							props.PrefixText = "<font color='#"..Color3.new(0.7, 0, 1):ToHex().."'>[FAPE PRIVATE]</font> "..message.PrefixText
...
runFunction(function()
	...
	local anims = {
        ...
        ["wtf"] = {
			{CFrame = CFrame.new(0.8, -5, 0.8) * CFrame.Angles(-math.rad(1910), math.rad(1110), -math.rad(901)), Time = 1.3},
			{CFrame = CFrame.new(1, 1, 1) * CFrame.Angles(math.rad(-910), math.rad(190), math.rad(190)),Time = 1.25}
		},
		["Exhibition Old"] = {
...
runFunction(function()
	local FieldOfViewValue1 = {Value = 70}
	local oldfov
	local oldfov2
	local FieldOfView1 = {Enabled = false}
	FieldOfView1 = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "BetterFOV",
		Function = function(callback)
			if callback then
				if FieldOfViewZoom.Enabled then
					task.spawn(function()
						repeat
							task.wait()
						until not inputService:IsKeyDown(Enum.KeyCode[FieldOfView1.Keybind ~= "" and FieldOfView1.Keybind or "C"])
						if FieldOfView1.Enabled then
							FieldOfView1.ToggleButton(false)
						end
					end)
				end
				oldfov = bedwars.FovController.setFOV
	...
	FieldOfViewValue1 = FieldOfView1.CreateSlider({
		Name = "FOVz",
		Min = 30,
		Max = 1000,
		Function = function(val)
...
runFunction(function()
	local NightMode = {}
	NightMode = GuiLibrary.CreateLegitModule({
		Name = "NightTime",
		Function = function(callback)
			if callback then
			...
			if callback then 
				...
				table.insert(AutoToxic.Connections, vapeEvents.LagbackEvent.Event:Connect(function(plr)
					if AutoToxicLagback.Enabled then
						warningNotification("Fape", "LMAO ".. plr.Name "FLAG LOL BRO HAS WORSE INTERNET OR SOMETHING LOLOLOOLOL", 5)
					end
				end))
